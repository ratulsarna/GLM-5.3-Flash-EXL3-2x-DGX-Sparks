...
ROUTER_FP32_PATCH_HOST="${ROUTER_FP32_PATCH_HOST:-$SCRIPT_DIR/overlay/patch_router_fp32_sm12x.py}"
...
# MoE router logits on SM12x (overlay/patch_router_fp32_sm12x.py). 1 = fp32
# product, the checkpoint's routing dtype; 0 = stock tier 6 (bf16-rounded).
GLM53_ROUTER_FP32="${GLM53_ROUTER_FP32-1}"
...
WORKER_STAGE="${WORKER_VLLM_CACHE:?}/stage-compact-dedup"
...
validate_numeric_config() {
    ...
    _glm53_validate_bool_flag GLM53_ROUTER_FP32 "${GLM53_ROUTER_FP32-1}" || return
...
validate_overlay_artifacts() {
    ...
    local -a artifacts=(
        ...
        "$ROUTER_FP32_PATCH_HOST|[glm53-router-fp32-sm12x]|$main_guard"
...
# editing that file. The SM12x fp32 router edits only gate_linear.py, which no
# other overlay touches.
GLM53_OVERLAY_ORDER=(
...
launch_cluster() {
    ...
    scp -q -o BatchMode=yes "$ROUTER_FP32_PATCH_HOST" "${WORKER_SSH}:${WORKER_STAGE}/patch_router_fp32_sm12x.py"
    ...
    for v in SERVED_MODEL_NAME PORT TP NNODES HEAD_IP MASTER_PORT QUANTIZATION \
             ...
             GLM53_EXL3_MOE_FAST GLM53_ROUTER_FP32 GLM53_KDA_BF16_LARGE_M \
    ...
    worker_ssh "docker run -d --restart no --name '$CONTAINER_WORKER' \
        ...
        -v '${WORKER_STAGE}/patch_router_fp32_sm12x.py:/opt/glm53/patch_router_fp32_sm12x.py:ro' \
    ...
    VLLM_API_KEY="$VLLM_API_KEY" docker run -d --restart no --name "$CONTAINER_HEAD" \
        ...
        -v "$ROUTER_FP32_PATCH_HOST:/opt/glm53/patch_router_fp32_sm12x.py:ro" \
        ...
        -e GLM53_ROUTER_FP32="$GLM53_ROUTER_FP32" \
